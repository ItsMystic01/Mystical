...
public class RemoveRank implements CommandExecutor {
    ...
    public RemoveRank(Mystical plugin, PlayerInfoManager playerInfoManager, RanksManager ranksManager, FileConfiguration langFile) {
        this.PLUGIN = plugin;
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (!playerSender.hasPermission(MysticalPermission.permissionENUM.REMOVE_RANK.getPermission())) { playerSender.sendMessage(
        ...
        if (args.length < 2) { playerSender.sendMessage(ChatColor.translateAlternateColorCodes('&',
                String.format(Objects.requireNonNull(langMessage), "/removeRank <player> <rank name>"))); return true; }
